...
```mermaid
flowchart LR
    User[用户] -->|浏览器| Frontend[管理后台 HTML/JS]
    User -->|对话框| Dify[Dify WebApp]
    
    Frontend -->|REST API| Backend[Spring Boot API]
    Dify -.->|HTTP 调用| Backend
    
    Backend --> MySQL[(MySQL)]
    Backend --> Redis[(Redis)]
    Backend --> Actuator[Actuator 监控]
    
    Dify --> Knowledge[知识库（服务目录/FAQ）]
    Dify --> LLM[DeepSeek / Ollama]
    Dify --> Rerank[Qwen Rerank]
🛠️ 技术栈
类别	技术
...
| 工单助手对话 | 管理后台看板 | 实时监控 |
|:---:|:---:|:---:|
| ![对话](https://raw.githubusercontent.com/laofu00/ticket-api/master/doc/image/chat.png) | ![后台](https://raw.githubusercontent.com/laofu00/ticket-api/master/doc/image/dashboard.png) | ![监控](https://raw.githubusercontent.com/laofu00/ticket-api/master/doc/image/monitor.png) |

| 工单列表管理 |
|:---:|
| ![工单列表](https://raw.githubusercontent.com/laofu00/ticket-api/master/doc/image/ticketList.png) |

截图保存在 doc/image/ 目录中，请根据实际文件名调整。
...
2. 克隆项目
bash
git clone https://github.com/yourname/it-ticket-assistant.git
cd it-ticket-assistant
3. 配置环境变量
...
/manage/loggers	动态调整日志级别（需配置 include）	POST /manage/loggers/com.ticket
生产环境建议使用监控系统（如 Prometheus + Grafana）拉取 /actuator/prometheus（需添加 micrometer-registry-prometheus）。
